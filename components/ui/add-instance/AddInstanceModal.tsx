...
export interface AddInstanceModalProps {
  ...
  assetsProgress?: { done: number; total: number; downloaded: number; failed: number; bytesDownloaded?: number; totalBytes?: number };
  error?: string | null;
  theme?: InkUITheme;
}
...
export const AddInstanceModal: React.FC<AddInstanceModalProps> = ({
  ...
  const isNameValid = name.trim().length >= 3 && name.trim().length <= 32 && !isDuplicate && !versionsLoading;

  useInput(
    (char, key) => {
      if (!focus || isLoading) return;
      // Esc and Tab are handled by App (global)
      if (key.tab) { setActiveField(p => p === 'name' ? 'version' : 'name'); return; }
      // Version field: only Select handles arrows/Enter, don't process anything here
      if (activeField === 'version') return;
      if (key.return && isNameValid) { onConfirm({ name: name.trim(), version: selectedVersion }); return; }
    ...
    <Box flexDirection="column" alignItems="center" justifyContent="center" flexGrow={1}>
      <Box borderStyle="round" borderColor={theme.colors.focus} paddingX={3} paddingY={1} flexDirection="column" gap={1} width={56}>
        ...
        <Box flexDirection="column" gap={1}>
          <Text color={versionFocused ? theme.colors.focus : theme.colors.muted} bold={versionFocused}>⬡ Minecraft {versionFocused ? '●' : ''}</Text>
          {versionsLoading ? (
        ...
        <Box marginTop={1} gap={2} justifyContent="center">
          <Text backgroundColor={isLoading ? 'yellow' : isNameValid ? 'green' : undefined} color={isLoading ? 'black' : isNameValid ? 'black' : 'gray'} bold={isNameValid || isLoading}>
        ...
        {isLoading && (phase === 'downloading-assets' || phase === 'downloading-libraries' || phase === 'downloading-client') && assetsProgress && (
          <Box flexDirection="column" gap={1} marginTop={1} borderStyle="single" borderColor={theme.colors.border} paddingX={1} paddingY={1}>
            <ProgressBar width={40} value={(assetsProgress.downloaded / Math.max(1, assetsProgress.total)) * 100} label={phase === 'downloading-assets' ? 'assets' : phase === 'downloading-libraries' ? 'libraries' : 'client'} />
            <Box justifyContent="space-between">
              <Text dimColor>
                {assetsProgress.downloaded}/{assetsProgress.total} files
                {assetsProgress.failed > 0 && <Text color="red"> · {assetsProgress.failed} failed</Text>}
              </Text>
              {assetsProgress.bytesDownloaded !== undefined && assetsProgress.totalBytes !== undefined && assetsProgress.totalBytes > 0 && (
                <Text dimColor>{(assetsProgress.bytesDownloaded / 1024 / 1024).toFixed(1)}/{(assetsProgress.totalBytes / 1024 / 1024).toFixed(1)} MB</Text>
              )}
            </Box>
          </Box>
        )}
