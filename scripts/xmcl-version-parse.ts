...
console.log(`Expected JSON path: ${path.join(minecraftLocation, "versions", minecraftVersionId, `${minecraftVersionId}.json`)}`);
console.log("");
...
try {
  ...
  console.log(`  first 3: ${resolved.libraries.slice(0, 3).map((l) => l.name).join(", ")}`);
  console.log(`downloads.client: ${resolved.downloads.client?.url ? "present" : "missing"} (${resolved.downloads.client?.url?.slice(0, 60)}...)`);
  console.log(`assetIndex: ${resolved.assetIndex?.id} -> ${resolved.assetIndex?.url?.slice(0, 60)}...`);
  ...
  console.log(`  maps to Version.parse(path.join(getInstancePath(id), ".minecraft") OR simply getInstancePath(id) as minecraftLocation, version)`);
  console.log(`  Example: Version.parse("${path.join(os.homedir(), ".local/share/craftty-dev/instances/my-instance")}", "${minecraftVersionId}")`);
  console.log("");
