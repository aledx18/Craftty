...
import { ensureSharedPath } from "../instanceFiles.js";
...
export async function installAssets(
  ...
  const assetsIndexPath = path.join(minecraftLocation, "assets", "indexes", `${version.assets}.json`);
  ...
  if (!fs.existsSync(assetsIndexPath)) {
    ...
    const buf = Buffer.from(await fetchWithTimeout(version.assetIndex.url, { signal }).then((r) => {
  ...
  if (missing.length === 0) {
    return { total: entries.length, downloaded: 0, skipped, failed: [], bytesDownloaded: 0, totalBytes };
  }
  ...
  async function worker() {
    while (idx < missing.length) {
      ...
      try {
        ...
        onProgress?.({ done, total: missing.length, downloaded, failed: failed.length, name: entry.name, bytes: bytesDownloaded });
      }
    }
  }
...
export async function installLibraries(
  ...
  async function downloadOne(lib: typeof libs[number]) {
    ...
    const buf = Buffer.from(await res.arrayBuffer() as any);
    const sha1 = crypto.createHash("sha1").update(buf).digest("hex");
    if (lib.download.sha1 && sha1 !== lib.download.sha1) throw new Error(`SHA1 mismatch for ${lib.name}`);
    fs.writeFileSync(dest, buf);
  }
  ...
  async function worker() {
    while (idx < missing.length) {
      ...
      try {
        ...
        onProgress?.({ done, total: missing.length, downloaded, failed: failed.length, name: lib.name });
      }
    }
  }
...
async function extractNatives(minecraftLocation: string, version: ResolvedVersion) {
  const natives = version.libraries.filter((lib) => lib.isNative || lib.download.path.includes("natives"));
  ...
  for (const lib of natives) {
    ...
    try {
      ...
      if (lib.download.sha1 && sha1 !== lib.download.sha1) {
        console.warn(`Native jar SHA1 mismatch, skipping extract: ${lib.name} (expected ${lib.download.sha1}, got ${sha1})`);
        continue;
      }
    } catch {
      continue;
